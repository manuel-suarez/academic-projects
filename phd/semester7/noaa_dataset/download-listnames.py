...
def get_products(product_name, outputpath, products_filter=None):
    json_ = requests.get(
        f"https://catalogue.dataspace.copernicus.eu/odata/v1/Products?$filter=Name eq '{product_name}.SAFE'"
    ).json()
    # print(json_)
                    ...
                    try:
                        ...
                        subprocess.run(
                            [
                                ...
                                f"manuelsuarez@siimon5.cimat.mx:/home/mariocanul/image_storage/dataset-tarso/sentinel1/{subdir}",
                            ]
                        )
...
def open_and_search_products(inputfile, outputpath, products_filter):
    products_ds = pd.read_csv(inputfile)
    total_rows = len(products_ds)
    print("Total rows: ", total_rows)

    for index, row in tqdm(products_ds.iterrows()):
        product_name = row["name"]
        print(f"Row {index} of {total_rows}, name: {product_name}")
        get_products(product_name, outputpath, products_filter)
...
if __name__ == "__main__":
    ...
    data_path = os.path.join(base_path, "data", "cimat", "dataset-tarso")
    input_file = os.path.join(data_path, "tarso_sentinel1_products.csv")
    output_path = os.path.join(data_path, "sentinel1_products")

    # Open CSV file with geopandas to get access to dates and coordinates

    open_and_search_products(input_file, output_path, ["GRD"])
