...
# Define auxiliary class for output evaluation (gradcam)
...
def generate_gradcam(model, image, label):
    ...
    # print(model)
    target_layers = [model.segmentation]
    targets = [SemanticSegmentationTarget(prediction_mask_float.squeeze(0).squeeze(0))]
...
if __name__ == "__main__":
    ...
    figures_path = os.path.join(
        args.figures_path, encoder_name, model_name, feat_channels
    )
    ...
    f = open(
        os.path.join("outputs", encoder_name, model_name, feat_channels, "gradcam.txt"),
        "x",
    )
    f.close()
