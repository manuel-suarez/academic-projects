...
parser.add_argument(
    "--feat_channels", type=str, required=True, default="o", help="features to use"
)
...
def print_metric_results(metric, step):
    ...
    encoders_dicts = {
        encoder_name(encoder, layer): [] for encoder, layer in product(encoders, layers)
    }
    table_results.update(encoders_dicts)
    for model, encoder, layer in product(models, encoders, layers):
        if model_names[model] not in table_results["Models"]:
            table_results["Models"].append(model_names[model])
        encoder_layer = f"{encoder}{layer}"
        metrics_path = os.path.join("metrics", encoder_layer, model, feat_channels)
        # Open metrics file and read results
        metrics_file = os.path.join(metrics_path, f"metrics.csv")
        if not os.path.exists(metrics_file):
            table_results[encoder_name(encoder, layer)].append("NA")
            continue
        # Open CSV
        ...
        table_results[encoder_name(encoder, layer)].append(metric_encoder)
