...
class Trainer:
    ...
    def zeros_step(self, outcomes=outcomes, metrics=metrics, thresholds=thresholds):
    ...
    def fit(
        ...
        training_metrics = initialize_metrics(
            {"start_time": []}, ["train", "test"], roc_metrics=False
        )
        training_losses = {"train": 0.0, "test": 0.0}
        ...
        for epoch in range(self.epochs):
            ...
            for step in ["train", "test"]:
                ...
                training_losses[f"{step}"] += epoch_outputs[step]["loss"]
                # Register outcomes
                for outcome in outcomes:
            ...
            metrics_msg = f"Epoch {epoch + 1}/{self.epochs}, LR: {self.scheduler.get_last_lr()[0]:.12f}"
            # Train metrics
            metrics_msg += f", train loss: {train_outputs['loss']:.8f}, train precision: {train_outputs['metrics']['precision']:.8f}, train recall: {train_outputs['metrics']['recall']}, train f1score: {train_outputs['metrics']['f1score']}, train miou: {train_outputs['metrics']['m_iou']:.8f}, train AUC: {train_outputs['metrics']['auc']:.8f}, train time: {train_outputs['time']-epoch_start_time:.2f}"
            # Valid metrics
            # metrics_msg += f", valid loss: {valid_outputs['loss']:.8f}, valid precision: {valid_outputs['metrics']['precision']:.8f}, valid recall: {valid_outputs['metrics']['recall']}, valid f1score: {valid_outputs['metrics']['f1score']}, valid miou: {valid_outputs['metrics']['m_iou']:.8f}, valid AUC: {valid_outputs['metrics']['auc']:.8f}, valid time: {valid_outputs['time']-valid_outputs['time']:.2f}"
            # Test metrics
            metrics_msg += f", test loss: {test_outputs['loss']:.8f}, test precision: {test_outputs['metrics']['precision']:.8f}, test recall: {test_outputs['metrics']['recall']}, test f1score: {test_outputs['metrics']['f1score']}, test miou: {test_outputs['metrics']['m_iou']:.8f}, test AUC: {test_outputs['metrics']['auc']:.8f}, test time: {test_outputs['time']-test_outputs['time']:.2f}"
            # Max test metrics
            if test_outputs["metrics"]["f1score"] > max_f1score_value:
                max_f1score_value = test_outputs["metrics"]["f1score"]
                max_f1score_epoch = epoch
                # Save best weights for f1score
                torch.save(
                    self.model.state_dict(),
                    os.path.join(self.weights_path, "weights_best_f1score.pth"),
                )
            if test_outputs["metrics"]["m_iou"] > max_miou_value:
                max_miou_value = test_outputs["metrics"]["m_iou"]
                max_miou_epoch = epoch
                # Save best weights for m_iou score
                torch.save(
                    self.model.state_dict(),
                    os.path.join(self.weights_path, "weights_best_miou.pth"),
                )
        ...
        training_losses["train"] /= self.epochs + 1
        # training_losses["valid"] /= self.epochs
        training_losses["test"] /= self.epochs + 1
        ...
        print(f"Max test F1-Score: {max_f1score_value} on epoch: {max_f1score_epoch}")
        print(f"Max test mIoU: {max_miou_value} on epoch: {max_miou_epoch}")
